...
static t_whirlpool_ctx	ctx;


static const u_int64_t T[] =
{
...
{
	ft_memset(&ctx, 0, sizeof(t_whirlpool_ctx));
}

void	whirlpool_transform(u_int64_t *m)
{
	u_int64_t	k[8] = {0};
	u_int64_t	l[8] = {0};
	u_int64_t	s[8] = {0};
...
void	whirlpool_update(u_int8_t *data, size_t len)
{
	...
	while (++i < len)
	{
		...
		if (off % 64)
			continue ;
		whirlpool_transform((u_int64_t *)ctx.buffer);
		off = 0;
	}
}
...
void	whirlpool_final(u_int8_t *digest)
{
	...
	if (off >= 32)
		pad_len += 64;
	whirlpool_update(pad, pad_len);
	ctx.len -= pad_len;

	m = (u_int64_t *)ctx.buffer;
	m[4] = 0;
	m[5] = 0;
	m[6] = 0;
	m[7] = BSWAPQ(ctx.len * 8);
	whirlpool_transform(m);
